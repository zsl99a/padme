...
中国个人创作者在微信公众号开通"流量主 → 返佣商品"(100 粉门槛,佣金 90% 归流量主,微信抽 10%) + 视频号开通"橱窗"(0 粉门槛,保证金 100-1000 元可退) → 用 LLM 流水线批量生成"种草/测评/经验"软文 + 视频脚本 → 在公众号图文 / 视频号短视频 / 视频号直播间插入"返佣商品"卡片 → 用户下单后平台自动分账 → 微信支付收款 → 提现到银行卡。
...
| 维度 | 分数(0-10) | v2 权重 | 加权 |
| --- | --- | --- | --- |
| 启动成本(资金) | 9(0-1000 元保证金可退) | 0.15 | 1.35 |
| 启动成本(技能) | 7(公众号基础 + AI 写 + 选品眼光) | 0.05 | 0.35 |
| 首笔收入速度 | 8(发出去即有分账) | 0.15 | 1.20 |
...
| + 现实数据奖励 | +0.3(小皮妈妈 2300 单 4.8 万佣金案例 ≈ $670/月) | — | +0.30 |
| **总分** | — | — | **8.1** |

> **修正**:广告法合规边界严("最/第一/保证"等绝对化用语不可用)、选品质量参差需人工首轮过滤、佣金 10% 平台抽成 + 微信生态有"软文违规"风险 → **7.05**

决策:**排队**(与已落档的「公众号短剧 CPS」(7.45)+「小报童 AI 数字专栏」(7.65)互为补充,同公众号生态三条腿)
